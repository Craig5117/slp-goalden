...
router.get("/", withAuth, (req, res) => {
  ...
  Student.findAll({
    ...
    .then((studentData) => {
      ...
      const userIdComparison = students[0]
      ? students[0].user_id 
      : -1; 
        ...
        .then((studentGoalData) => {
          ...
          // replace student name and id with Student db call
            ...
            if (studentGoals[0] == null) {
                ...
                const userIdComparison = studentGoals[0]
                ? studentGoals[0].user_id
                : -1; 
                if (userIdComparison === userId) {
                  res.render("single-student", { studentGoals, studentName, studentId, loggedIn: req.session.loggedIn });
                } else {
...
router.get("/trial-submit/:id", withAuth, (req, res) => {
  StudentGoal.findOne({
    ...
    .then((studentGoalData) => {
      ...
      const userIdComparison = studentGoalData
      ? studentGoalData.user_id 
      : -1; 
